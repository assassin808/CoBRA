...
class RepEControlExperiment(ControlExperiment):
    ...
    def __init__(self, model, tokenizer, device, plot_dir, rep_control_pipeline, rep_reader, control_layers, is_testing_mode=False, num_permutations=5, experiment_suffix="", reasoning_temperature=0.7, choice_temperature=1.0):
        ...
        # Reasoning generation parameters - Updated for thinking mode
        self.reasoning_temperature = reasoning_temperature
        self.reasoning_num_samples = 5  # Changed from 3 to 5
        self.reasoning_max_tokens = 64   # Changed from MAX_NEW_TOKENS to 64 for thinking tokens
        self.reasoning_do_sample = True  # Enable sampling
    ...
    def _get_repe_choice_probabilities(self, prompts, batch_mode, mcq_prompt_addon, target_token_map, level=None, operator='linear_comb', mcq_scenario_key='', **kwargs):
        ...
        if batch_mode:
            ...
            for coeff, level_prompts in tqdm(prompts_by_level.items(), desc="Batch Generating Reasoning"):
                ...
                print(f"[INFO] Processing coefficient {coeff:.3f} with {len(level_prompts)} prompts")
                print(f"[INFO] Current reasoning batch size: {self.reasoning_batch_size}")
                ...
                for i in range(0, len(level_prompts), self.reasoning_batch_size):
                    ...
                    current_batch_size = len(batch_prompts_data)
                    print(f"[INFO] Processing batch {i//self.reasoning_batch_size + 1}: {current_batch_size} prompts (batch size limit: {self.reasoning_batch_size})")
                    ...
                    all_outputs = []
                    total_generations = current_batch_size * self.reasoning_num_samples
                    print(f"[INFO] Generating {total_generations} total outputs ({current_batch_size} prompts × {self.reasoning_num_samples} samples)")
                    ...
                    for sample_idx in range(self.reasoning_num_samples):
                        ...
                        print(f"[INFO] Sample {sample_idx + 1}/{self.reasoning_num_samples}: Processing {current_batch_size} prompts...")
                        
                        sample_outputs = self.rep_control_pipeline(
                            prompts_for_gen, 
                            activations=activations, 
                            max_new_tokens=self.reasoning_max_tokens,  # 64 tokens for thinking
                            do_sample=self.reasoning_do_sample,        # Enable sampling
                            temperature=self.reasoning_temperature,    # 0.7 temperature
                            num_return_sequences=1,  # Force to 1 since batch mode doesn't handle multiple properly
                            pad_token_id=self.tokenizer.eos_token_id, 
                            eos_token_id=self.tokenizer.convert_tokens_to_ids("<|im_end|>"), 
                            batch_size=current_batch_size  # Use actual batch size
                        )
                    ...
                    for j, p_data in enumerate(batch_prompts_data):
                        ...
                        # Store all reasoning variants AND prepare for multiple choice calculations
                        reasoning_variants = []
                        ...
                        # Store all variants for later use in choice calculation
                        p_data['reasoning_text'] = reasoning_variants[0]  # Primary variant
                        p_data['reasoning_variants'] = reasoning_variants  # All variants for multiple choice calculations
        ...
        # SINGLE MODE - Now handles multiple choice calculations
        p_data = prompts[0]
        coeff = p_data['level']
        ...
        # Get all reasoning variants (should have 5 from batch generation)
        reasoning_variants = p_data.get('reasoning_variants', [p_data.get('reasoning_text', "")])
        if not reasoning_variants or all(not variant.strip() for variant in reasoning_variants):
            reasoning_variants = [""]  # Fallback
        
        full_mcq_prompt = f"{p_data['base_prompt']}{p_data['options_block']}\n{mcq_prompt_addon}"

        # Calculate choice probabilities for each reasoning variant (up to 5 times)
        all_choice_probs = []
        for i, reasoning_text in enumerate(reasoning_variants):
            if mcq_scenario_key in ["choice_first", "choice_then_reasoning"]:
            ...
            choice_probs = get_controlled_choice_probabilities(
                self.rep_control_pipeline, self.tokenizer, prompt_for_prob_calc, 
                activations, target_token_map, self.device, 
                debug=self.is_testing_mode, operator=operator, 
                temperature=self.choice_temperature
            )
            all_choice_probs.append(choice_probs)
            
            if self.is_testing_mode:
                print(f"[DEBUG] Choice probs for reasoning variant {i+1}: {choice_probs}")
        
        # Average the choice probabilities across all reasoning variants
        if len(all_choice_probs) > 1:
            averaged_choice_probs = {}
            for choice_label in target_token_map.keys():
                probs = [cp.get(choice_label, 0.0) for cp in all_choice_probs]
                averaged_choice_probs[choice_label] = np.mean(probs)
            
            if self.is_testing_mode:
                print(f"[DEBUG] Averaged choice probs across {len(all_choice_probs)} variants: {averaged_choice_probs}")
            
            return averaged_choice_probs, reasoning_variants[0]  # Return primary reasoning text
        else:
            return all_choice_probs[0], reasoning_variants[0]

    def run(self, scenarios, mcq_options, prompt_template, scenario_name, operators=['linear_comb','projection', 'orthognalize']): # ,
        print("\n" + "="*50 + f"\n--- Running RepE Control on {scenario_name} Scenarios ---\n" + "="*50)
        
        # Print batch configuration
        from control.env_config import print_batch_config
        print_batch_config()
        print(f"Experiment parameters: {self.reasoning_num_samples} reasoning samples, {self.num_permutations} permutations")
        print(f"Temperature settings: reasoning={self.reasoning_temperature}, choice={self.choice_temperature}")
        ...
        for mcq_key, mcq_instruction in self.mcq_scenarios.items():
            ...
            for operator in operators:
                ...
                # Generalized xlabel so it applies to all bias types (authority, bandwagon, framing, confirmation)
                xlabel = f"Control Coefficient (Bias Direction, Operator: {operator})"
