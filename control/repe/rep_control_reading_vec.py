...
class WrappedBlock(torch.nn.Module):
    ...
    def set_controller(self, activations, token_pos=None, masks=None, normalize=False, operator='linear_comb'):
        ...
        if operator == 'linear_comb':
            ...
            def op(current, controller):
                # project the current activation onto the controller
                controller_norm = torch.norm(controller, dim=-1, keepdim=True)
                unit_controller = controller / (controller_norm + 1e-8)
                current_norm = torch.norm(current, dim=-1, keepdim=True)
                unit_current = current / (current_norm + 1e-8)
                if torch.isnan(unit_current).any() or torch.isnan(unit_controller).any():
                    unit_current = torch.nan_to_num(unit_current, nan=0.0)
                    unit_controller = torch.nan_to_num(unit_controller, nan=0.0)
                # Remove projection: current - <current, controller> * controller
                proj = (unit_current * unit_controller).sum(-1, keepdim=True) * unit_controller
                eliminated = current #- abs(proj) * current_norm
                # Optionally, add back a scaled controller (like projection operator)
                weight = (unit_current * unit_controller).sum(-1, keepdim=True)
                return eliminated + abs(weight) * controller * 10 * current_norm
        elif operator == 'orthognalize':
            def op(current, controller):
                # Normalize controller and current
                controller_norm = torch.norm(controller, dim=-1, keepdim=True)
                unit_controller = controller / (controller_norm + 1e-8)
                current_norm = torch.norm(current, dim=-1, keepdim=True)
                unit_current = current / (current_norm + 1e-8)
