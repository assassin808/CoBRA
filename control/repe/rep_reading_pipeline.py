...
# Import batch size from environment config
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from env_config import REP_READING_BATCH_SIZE
...
class RepReadingPipeline(Pipeline):
    ...
    def _get_hidden_states(
        ...
        # here we illustrate the structure of transformer suppose input is h_i at layer i 
        # the dimension of h_i is (batch_size, head_num, seq_len, d_k)
        # outputs['hidden_states'] is a list of hidden states for each layer
        # then we start the calculation of attention heads 
        # Q_i = W_Q_i * h_i
        # K_i = W_K_i * h_i
        # V_i = W_V_i * h_i
        # then we calculate the attention scores
        # A_i = softmax(Q_i * K_i^T / sqrt(d_k))
        # and finally we get the output of the layer
        # O_i = A_i * V_i
        # the next hidden state is calculated as
        # here we ignore the connection of the multiple heads
        # h_{i+1} = MLP(O_i) + h_i
        # (head_num, seq_len, d_k) 

        # here we illustrate how next token is calculated
        # 
    ...
    def get_directions(
            ...
            n_difference: int = 1,
            batch_size: int = REP_READING_BATCH_SIZE,  # Load from .env
