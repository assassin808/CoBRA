...
class PCARepReader(RepReader):
    ...
    def get_rep_directions(self, model, tokenizer, hidden_states, hidden_layers, **kwargs):
        ...
        for layer in hidden_layers:
            ...
            # directions[layer] = directions[layer] / np.linalg.norm(directions[layer], axis=1, keepdims=True)
        
        return directions
    def _plot_pca(self, H_train, layer, n_components=20):
        """
        Run PCA with a large number of components and plot the explained variance and the first few principal components.
        This is similar to how signal processing visualizes the frequency spectrum (Fourier transform).
        Args:
            H_train: Hidden states (n_samples, hidden_size)
            layer: Layer name or index (for labeling)
            n_components: Number of PCA components to compute and plot
        """
        import matplotlib.pyplot as plt
        from sklearn.decomposition import PCA
        import numpy as np

        # Ensure H_train is a numpy array
        if isinstance(H_train, torch.Tensor):
            H_train = H_train.cpu().numpy()

        # Run PCA
        pca = PCA(n_components=n_components)
        H_train_centered = H_train - np.mean(H_train, axis=0, keepdims=True)
        pca.fit(H_train_centered)

        # Plot explained variance ratio (scree plot)
        plt.figure(figsize=(10, 4))
        plt.plot(np.arange(1, n_components+1), pca.explained_variance_ratio_, marker='o')
        plt.title(f'Explained Variance Ratio by PCA Component (Layer {layer})')
        plt.xlabel('Component')
        plt.ylabel('Explained Variance Ratio')
        plt.grid(True)
        plt.show()

        # Plot the first few principal components as signals
        plt.figure(figsize=(12, 6))
        for i in range(min(5, n_components)):
            plt.plot(pca.components_[i], label=f'PC {i+1}')
        plt.title(f'First {min(5, n_components)} PCA Components (Layer {layer})')
